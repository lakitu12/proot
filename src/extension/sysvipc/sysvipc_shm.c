...
int sysvipc_shmctl(Tracee *tracee, struct SysVIpcConfig *config)
{
	...
	struct SysVIpcSharedMem *shm;
	LOOKUP_IPC_OBJECT(shm_index, shm, config->ipc_namespace->shms)
	(void) shm_index; // Suppress unused variable warning
...
void sysvipc_shm_helper_main() {
	...
	for (int i = 0;; i++) {
		...
		strcpy(temp_template, path);
		strcpy(temp_template + strlen(temp_template) - 6, "XXXXXX");  /* Replace 'XXXXXX' part */
		
		/* Use mkstemp to create a unique temporary file, then close and remove it */
		int fd = mkstemp(temp_template);
		if (fd != -1) {
			close(fd);
			unlink(temp_template);  /* Remove the file created by mkstemp */
		}
		
		/* Copy the template back to path */
		strcpy((char*)path, temp_template);

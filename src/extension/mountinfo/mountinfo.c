	...
	if (
		...
		while ((line_len = getline(&line, &line_buf_len, real_mountinfo_fp)) > 0) {
			...
			if (chunk_end - chunk == 5 && 0 == memcmp(chunk, "/data", 5)) {
				...
				(void) fwrite(line, chunk - line + 1, 1, new_mountinfo_fp);
				(void) fwrite(chunk_end, line_len - (chunk_end - line), 1, new_mountinfo_fp);
				found_line = true;
				break;
			}
end_line_scan: ;
		...
		if (found_line) {
			...
			while ((line_len = getline(&line, &line_buf_len, real_mountinfo_fp)) > 0) {
				...
				if (
					...
					(void) fwrite(line, line_len, 1, new_mountinfo_fp);
				}
end_line_scan2: ;
			}
		}
		...
		if (found_line) {
			(void)strncpy(path, new_path, PATH_MAX);
		}
		return;
	}

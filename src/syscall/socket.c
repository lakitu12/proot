...
#include <string.h>      /* strncpy(3), strlen(3), memset(3), */
...
int translate_socketcall_enter(Tracee *tracee, word_t *address, int size)
{
	...
	if (strlen(host_path) > sizeof_path) {
		...
		strcpy(temp_name, shorter_host_path);
		int fd = mkstemp(temp_name);
		if (fd != -1) {
			close(fd);
			unlink(temp_name);  /* Remove the file created by mkstemp */
		}

		/* Use the template path */
		strcpy(shorter_host_path, temp_name);
